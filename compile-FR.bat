...
docker build -t uvk5 .
docker run --rm -v %CD%\compiled-firmware:/app/compiled-firmware uvk5 /bin/bash -c "cd /app && make clean && make -s ENABLE_DEV=1 ENABLE_FR_BAND=1 ENABLE_EEPROM_512K=0 TARGET=robzyl8k_beta.fr && cp *packed.bin compiled-firmware/"
time /t
pause